...
- [x] [Review][Patch] `reivindicarVendido()`/`liberar()` sem guarda de posse (`reservaId`) no `WHERE` e sem checar o `int` de linhas afetadas — mutação de assento confiando só na aplicação, não no banco (viola non-negotiable de segurança das instruções do projeto) [api/src/main/java/br/com/rolo35/api/pagamentos/service/PagamentoService.java:83,89; api/src/main/java/br/com/rolo35/api/sessoes/repository/AssentoSessaoRepository.java:60-88] — **resolvido**: os dois métodos ganharam parâmetro `reservaId` no `WHERE`; `PagamentoService` checa o `int` retornado contra `assentoIds.size()` e lança `AssentoIndisponivelException` em caso de mismatch. Testes novos em `PagamentoServiceTest` cobrem os dois caminhos (aprovado/recusado).
- [x] [Review][Patch] Timeout de lock ao travar `Reserva` (`findByIdForUpdate`) não é capturado — cai no handler genérico de `PessimisticLockingFailureException` (503 `SALA_OCUPADA`, mensagem de criação de sessão), em vez de um erro específico de pagamento como `ReservaService.reservar()` já faz com `AssentoEmDisputaException` [api/src/main/java/br/com/rolo35/api/pagamentos/service/PagamentoService.java:54,57] — **resolvido**: novo `ReservaEmDisputaException` (409 `RESERVA_EM_DISPUTA`), capturado no `catch` ao redor de `findByIdForUpdate`, mesmo padrão de `ReservaService.reservar()`/`AssentoEmDisputaException`.
- [x] [Review][Patch] `CodigoIngressoService` não valida secret vazio/curto no construtor — um `TICKET_HMAC_SECRET` em branco constrói a chave silenciosamente em vez de falhar rápido no boot [api/src/main/java/br/com/rolo35/api/ingressos/service/CodigoIngressoService.java:21-22] — **resolvido**: construtor lança `IllegalStateException` para secret `null`/em branco, falha rápida no boot.
- [ ] [Review][Patch] `validar()`/`extrairId()` lançam NPE se `codigo` for `null`, sem guarda defensiva [api/src/main/java/br/com/rolo35/api/ingressos/service/CodigoIngressoService.java:29-30,47-48]
- [ ] [Review][Patch] `IngressoRepositorySmokeTest` não limpa os fixtures que cria (`Reserva`/`Ingresso`), diferente dos demais testes de integração da story — acumula linhas órfãs no Testcontainer reutilizado (`.withReuse(true)`) [api/src/test/java/br/com/rolo35/api/ingressos/IngressoRepositorySmokeTest.java]
- [ ] [Review][Patch] `reivindicarVendido()` limpa o persistence context (`clearAutomatically`) logo após `ingressoRepository.save()`, antes de montar os DTOs — funciona hoje porque só toca campos já materializados, mas é frágil pra mudanças futuras; mover `paraDto()` antes da chamada elimina o risco sem custo [api/src/main/java/br/com/rolo35/api/pagamentos/service/PagamentoService.java:78-84]
- [ ] [Review][Patch] `findByReservaId()` sem `ORDER BY` — ordem dos ingressos no caminho idempotente (`montarResposta`) pode variar entre chamadas [api/src/main/java/br/com/rolo35/api/ingressos/repository/IngressoRepository.java]
- [x] [Review][Defer] Autorização de papel só no controller (`@PreAuthorize`), sem checagem própria no service — deferred, pre-existing (mesmo padrão de `ReservaService` em todo o projeto)
- [x] [Review][Defer] `Ingresso.createdAt` (`Instant`) e `Ingresso.validatedAt` (`LocalDateTime`) misturam tipos temporais — deferred, pre-existing (mesma divisão já usada em `Reserva`)
- [x] [Review][Defer] `lock_timeout = '3s'` hardcoded na query nativa, não configurável via `application.properties` — deferred, pre-existing (mesmo literal já usado em `ReservaService.reservar()`)
...
- **`TICKET_HMAC_SECRET` é um segredo novo, não reaproveita `JWT_SECRET`.** Non-negotiable explícito de AD-8/instruções do projeto — os dois protegem coisas diferentes (sessão de usuário vs. autenticidade de ingresso físico/QR) e um vazamento de um não deve comprometer o outro. Mesmo padrão de leitura (`@Value` sem fallback, obrigatório) do `security.jwt.secret` já usado por `JwtService`.
...
- [Source: instruções do projeto — código do ingresso assinado e não forjável, segredos só em variável de ambiente, Metodologia XP + TDD, Convenções de nomenclatura]
