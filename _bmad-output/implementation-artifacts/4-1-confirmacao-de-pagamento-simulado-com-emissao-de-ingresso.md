...
- [x] [Review][Patch] `reivindicarVendido()`/`liberar()` sem guarda de posse (`reservaId`) no `WHERE` e sem checar o `int` de linhas afetadas — mutação de assento confiando só na aplicação, não no banco (viola non-negotiable de segurança do CLAUDE.md) [api/src/main/java/br/com/rolo35/api/pagamentos/service/PagamentoService.java:83,89; api/src/main/java/br/com/rolo35/api/sessoes/repository/AssentoSessaoRepository.java:60-88] — **resolvido**: os dois métodos ganharam parâmetro `reservaId` no `WHERE`; `PagamentoService` checa o `int` retornado contra `assentoIds.size()` e lança `AssentoIndisponivelException` em caso de mismatch. Testes novos em `PagamentoServiceTest` cobrem os dois caminhos (aprovado/recusado).
