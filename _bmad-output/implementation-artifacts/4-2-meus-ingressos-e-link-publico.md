...
- [x] [Review][Patch] `IngressoRepository.buscarPorCliente()` filtra `WHERE r.clienteId = :clienteId` e faz `JOIN` por `reservaId`/`assentoId`/`sessaoId`/`salaId`, mas nenhuma dessas colunas tem índice — viola o non-negotiable "Índices nas colunas usadas em filtros/joins das telas" das instruções do projeto, e é exatamente o cenário que `deferred-work.md` (Story 3.2) já tinha previsto ("revisitar quando uma consulta de produção [em `reservas.cliente_id`] existir, ex.: Epic 4") [api/src/main/resources/db/migration/; api/src/main/java/br/com/rolo35/api/ingressos/repository/IngressoRepository.java:16-28] — **resolvido**: `V4__indices_ingressos_por_cliente.sql` adiciona índice em `reservas.cliente_id` e `ingressos.reserva_id` (os dois pontos reais de filtro/join da query; `assento_id`/`sessao_id`/`sala_id` já são lookups por PK, indexados automaticamente).
- [x] [Review][Patch] `IngressoResumoProjection.getStatus()` tipado `String` força `StatusIngresso.valueOf(p.getStatus())` no service — conversão manual que lança `IllegalArgumentException` não tratada (500) se o valor do banco não bater com o enum; Spring Data suporta getter de projeção tipado direto como enum pra um campo `@Enumerated` selecionado via JPQL, eliminando a conversão manual [api/src/main/java/br/com/rolo35/api/ingressos/repository/IngressoResumoProjection.java:9; api/src/main/java/br/com/rolo35/api/ingressos/service/IngressoService.java:47] — **resolvido**: `getStatus()` da projeção agora retorna `StatusIngresso` direto; `IngressoService` não converte mais manualmente (Spring Data mapeia o alias JPQL `i.status`, já tipado `StatusIngresso` na entidade, sem mudança na query).
- [x] [Review][Patch] `ORDER BY s.dataHora DESC` sem desempate — ordem de ingressos com o mesmo `dataHora` de sessão pode variar entre chamadas [api/src/main/java/br/com/rolo35/api/ingressos/repository/IngressoRepository.java:28] — **resolvido**: `ORDER BY s.dataHora DESC, i.createdAt DESC` — ingresso mais recente da mesma sessão aparece primeiro, de forma determinística. Teste novo prova o desempate invertendo a ordem de inserção contra a ordem esperada (evita falso-positivo por coincidir com a ordem física de inserção).
- [x] [Review][Patch] `IngressoSecurityTest` cobre `CLIENTE`/sem token/`ORGANIZADOR` pra `/minhas`, mas nunca testa o papel `PORTARIA` — terceiro papel real do domínio, sem cobertura nesta rota [api/src/test/java/br/com/rolo35/api/ingressos/IngressoSecurityTest.java] — **resolvido**: `minhasReturns403ForPortariaToken` fecha a lacuna, confirma o comportamento já correto de `@PreAuthorize("hasRole('CLIENTE')")`.
- [x] [Review][Patch] Nome de teste mistura inglês/português (`buscarPublicoReturns200WithoutTokenNenhum`) — resto do arquivo é consistente, esse parece resíduo de edição [api/src/test/java/br/com/rolo35/api/ingressos/IngressoSecurityTest.java] — **resolvido**: renomeado pra `buscarPublicoReturns200WithoutToken`.
- [x] [Review][Patch] Nenhum teste prova explicitamente que `buscarPublico()` nunca muta estado (AC4) — a ausência de mutação hoje é só estrutural (não existe chamada de escrita no método); um `verify(ingressoRepository, never()).save(any())` fecharia essa lacuna antes da Story 5.2 (validação de portaria) pousar no mesmo pacote [api/src/test/java/br/com/rolo35/api/ingressos/service/IngressoServiceTest.java] — **resolvido**: `buscarPublicoNuncaMutaEstado` torna a garantia explícita.
- [x] [Review][Defer] `buscarPublico()` faz 3 consultas sequenciais (`Ingresso`→`Sessao`→`Sala`) em vez de um `JOIN` único como `buscarPorCliente()` — deferred, tradeoff aceitável pra uma rota pública de baixo tráfego (sempre 3 consultas fixas, não N+1 real por linha)
- [x] [Review][Defer] `permitAll()` de `/api/ingressos/*` em `SecurityConfig` é um catch-all que qualquer rota `GET` futura de um segmento só herdaria como pública por padrão — deferred, tradeoff deliberado já documentado em `docs/decisions.md` (ordem dos matchers)
- [x] [Review][Defer] Side-channel de tempo entre "assinatura inválida" (rápido, sem banco) e "assinatura válida mas não encontrado" (mais lento, com banco) em `buscarPublico()` — deferred, fora do modelo de ameaça declarado pela AC5 (que é sobre conteúdo da resposta, não tempo de resposta)
- [x] [Review][Defer] `listarMinhas()` sem paginação — deferred, fora do escopo declarado do desafio de 7 dias
- [x] [Review][Defer] `IngressoService.listarMinhas()`/`buscarPublico()` sem `@Transactional(readOnly = true)` — deferred, mesmo padrão já usado pelos métodos de leitura de `SessaoService` (`listarMinhas`, `listarPublicadas`, `mapaAssentos`), nenhum deles anotado
...
- **Rota pública nunca adquire lock nem escreve — reforço explícito de AD-9.** Ao contrário de toda outra leitura autenticada do projeto que eventualmente evolui pra escrita (ex.: `SessaoService.mapaAssentos()` é hoje só leitura mas convive no mesmo service que `criar()`/`editar()`, que travam), o método `buscarPublico()` desta story não deve ganhar nenhum parâmetro ou branch de escrita no futuro — validação de portaria (Story 5.2) é deliberadamente uma rota e um método de service **totalmente separados**, nunca uma extensão deste. Isso não é otimização prematura, é o non-negotiable de segurança das instruções do projeto ("link de compartilhamento não permite bypass da validação de portaria").
...
- [Source: instruções do projeto — Non-negotiable "link de compartilhamento somente leitura, sem bypass de portaria", Metodologia XP + TDD, evitar N+1]
