...
- [ ] [Review][Patch] 409 no front limpa a seleção inteira, não só os assentos que ficaram indisponíveis — o Task 7 original pedia "limpar a seleção de qualquer assento que não esteja mais livre" (limpeza seletiva); a implementação faz `setSelecionados(new Set())` incondicional, forçando reselecionar tudo mesmo quando só 1 de 6 assentos foi perdido. O teste novo só cobre seleção de 1 assento, não distingue os dois comportamentos. Confirmado ainda presente em 2026-08-12 (linha 141 de `MapaAssentosPage.tsx`), mesmo depois do commit do layout de auditório. [`web/src/pages/MapaAssentosPage.tsx` — bloco 409 de `handleReservar`]
- [ ] [Review][Patch] Recarga do mapa após 409 não tem guarda de componente desmontado — o `useEffect` inicial usa a flag `ativo` (via `carregarMapa(() => ativo)`), mas a chamada de `carregarMapa()` dentro do catch 409 de `handleReservar` usa o default `aindaAtivo = () => true`, então `setMapa`/`setEstado` podem disparar depois do componente desmontar (ex.: usuário navega de volta enquanto a recarga está em voo). Confirmado ainda presente em 2026-08-12 (linha 142). [`web/src/pages/MapaAssentosPage.tsx` — bloco 409 de `handleReservar`]
- [ ] [Review][Patch] Sem guarda de reentrância síncrona em `handleReservar` — depende só do `disabled={... || reservando}` do botão, que só reflete depois de um re-render; um duplo clique/Enter muito rápido pode disparar `reservarAssentos` duas vezes antes do primeiro `setReservando(true)` surtir efeito visual. Sem risco de dado (o backend já rejeita a segunda tentativa via lock), mas gera um flash de erro desnecessário. Confirmado ainda presente em 2026-08-12 (início de `handleReservar`, linha 124). [`web/src/pages/MapaAssentosPage.tsx` — `handleReservar`]
