...
- [x] [Review][Decision→Patch] Corrida TOCTOU no check de hold ativo de `editar()` reabria a brecha que a Task 5 fechou — `SessaoService.editar()` lia `assentoSessaoRepository.findByIdSessaoId(id)` **sem lock** antes de checar hold ativo e chamar `deleteAll`; nenhuma linha de `assento_sessao` era travada nesse trecho, diferente de `Sala`/`Sessao` (que já usam `findByIdForUpdate`). Uma chamada concorrente a `ReservaService.reservar()` podia confirmar um hold novo *depois* da leitura de `editar()` e *antes* do `deleteAll`, deixando a reserva órfã. **Resolvido**: Felipe decidiu corrigir imediatamente (severidade avaliada como média — janela estreita, mas viola o non-negotiable das instruções do projeto e desfaz o objetivo da Task 5). Novo método `AssentoSessaoRepository.travarPorSessao(sessaoId)` (`@Lock(PESSIMISTIC_WRITE)`, `ORDER BY assento_id`, mesmo mecanismo AD-3 de `travarParaReserva`) substitui o `findByIdSessaoId` desprotegido em `editar()`. Prova por banco real: `ReservaEditarConcorrenciaTest` (duas conexões reais disputando o mesmo assento) confirma que exatamente uma das duas operações vence, nunca as duas, nunca reserva órfã. [`api/src/main/java/br/com/rolo35/api/sessoes/service/SessaoService.java:158`, `api/src/main/java/br/com/rolo35/api/sessoes/repository/AssentoSessaoRepository.java`, `api/src/test/java/br/com/rolo35/api/reservas/ReservaEditarConcorrenciaTest.java`]
- [x] [Review][Patch] `reivindicar()` não tem guarda de status no próprio UPDATE (defesa em profundidade) — a query trava via `travarParaReserva` (PESSIMISTIC_WRITE) e o `ReservaService` só confirma que o status está efetivamente livre antes de chamar `reivindicar()`; isso já é seguro no único call site atual, mas o `UPDATE` em si não tem `WHERE status = 'LIVRE' OR (status = 'RESERVADO' AND expires_at < :agora)` — um futuro call site de `reivindicar()` que pule a checagem prévia sobrescreveria um assento vendido/reservado sem o banco reclamar. **Resolvido**: `reivindicar()` ganhou o `WHERE` de guarda e passou a devolver `int` (linhas afetadas); `ReservaService.reservar()` agora lança `AssentoIndisponivelException` se o número de linhas afetadas não bater com a seleção. Prova por banco real: `reivindicarNaoSobrescreveAssentoJaVendidoMesmoSemChecagemPrevia` (Testcontainers) chama `reivindicar()` sem nenhuma checagem prévia em Java e confirma que um assento `VENDIDO` não é sobrescrito. [`api/src/main/java/br/com/rolo35/api/sessoes/repository/AssentoSessaoRepository.java:41-56`, `api/src/main/java/br/com/rolo35/api/reservas/service/ReservaService.java:79-84`, `api/src/test/java/br/com/rolo35/api/sessoes/repository/ReservaAssentoLockRepositoryTest.java`]
- [x] [Review][Patch] Timeout de lock em `ReservaService.reservar()` não tem tratamento dedicado — `travarParaReserva()` roda sem try/catch; se o `PESSIMISTIC_WRITE` estourar o `lock_timeout` de 3s, a exceção cai no handler genérico de `SALA_OCUPADA` (mensagem errada, fala de "criação de sessão") ou em `handleGeneric` (500 opaco). **Resolvido, com desenho revisado**: em vez de relançar como `AssentoIndisponivelException` (409, "confirmado indisponível") como o achado original sugeria, criada `AssentoEmDisputaException` dedicada (409, código `ASSENTO_EM_DISPUTA`) — decisão de Felipe em debate: timeout de lock é incerteza de contenção, não indisponibilidade confirmada; misturar os dois enganaria o front sobre se vale a pena tentar de novo. Decisão registrada em `docs/decisions.md`. [`api/src/main/java/br/com/rolo35/api/reservas/service/ReservaService.java:58-66`, `api/src/main/java/br/com/rolo35/api/reservas/AssentoEmDisputaException.java`, `api/src/main/java/br/com/rolo35/api/common/GlobalExceptionHandler.java`]
- [x] [Review][Defer] 409 no front limpa a seleção inteira, não só os assentos que ficaram indisponíveis — o Task 7 original pedia "limpar a seleção de qualquer assento que não esteja mais livre" (limpeza seletiva); a implementação faz `setSelecionados(new Set())` incondicional, forçando reselecionar tudo mesmo quando só 1 de 6 assentos foi perdido. Confirmado ainda presente em 2026-08-12. **Descartado** por decisão de Felipe ao fechar a story — sem risco de dado, é degradação de UX menor (reselecionar tudo em vez de só o assento perdido). [`web/src/pages/MapaAssentosPage.tsx` — bloco 409 de `handleReservar`]
- [x] [Review][Defer] Recarga do mapa após 409 não tem guarda de componente desmontado — o `useEffect` inicial usa a flag `ativo` (via `carregarMapa(() => ativo)`), mas a chamada de `carregarMapa()` dentro do catch 409 de `handleReservar` usa o default `aindaAtivo = () => true`, então `setMapa`/`setEstado` podem disparar depois do componente desmontar. Confirmado ainda presente em 2026-08-12. **Descartado** por decisão de Felipe ao fechar a story — janela estreita (precisa desmontar exatamente durante a recarga pós-409), sem risco de dado, só um warning de React em caso raro. [`web/src/pages/MapaAssentosPage.tsx` — bloco 409 de `handleReservar`]
- [x] [Review][Defer] Sem guarda de reentrância síncrona em `handleReservar` — depende só do `disabled={... || reservando}` do botão, que só reflete depois de um re-render; um duplo clique/Enter muito rápido pode disparar `reservarAssentos` duas vezes antes do primeiro `setReservando(true)` surtir efeito visual. Confirmado ainda presente em 2026-08-12. **Descartado** por decisão de Felipe ao fechar a story — sem risco de dado (o backend já rejeita a segunda tentativa via lock), só um flash de erro desnecessário. [`web/src/pages/MapaAssentosPage.tsx` — `handleReservar`]
- [x] [Review][Patch] `ClienteNaoEncontradoException` → `401 NAO_AUTENTICADO` nunca é exercitado por teste de controller — `ReservaControllerTest` cobre 200/400/409, mas não o caminho "usuário do token sumiu", apesar do handler dedicado existir em `GlobalExceptionHandler`. **Resolvido**: novo teste `returns401WithNaoAutenticadoEnvelopeWhenServiceRejectsClienteNaoEncontrado`. [`api/src/test/java/br/com/rolo35/api/reservas/controller/ReservaControllerTest.java`]
- [x] [Review][Patch] `ReservaRepositorySmokeTest` não confere round-trip de `createdAt` — a Task 1 prometia "assert de round-trip dos campos", mas só `clienteId`/`sessaoId`/`status`/`expiresAt` são verificados. **Resolvido**: assert de `createdAt` adicionado, truncado a microssegundos (precisão máxima de `TIMESTAMP` sem timezone no Postgres). [`api/src/test/java/br/com/rolo35/api/reservas/ReservaRepositorySmokeTest.java`]
...
- **`ReservaDto` não inclui `clienteId` no corpo de resposta.** O próprio cliente autenticado já sabe quem é (é o dono do token); devolver o id de volta não agrega nada e amplia a superfície de dado exposta sem necessidade (mesmo espírito de AD-12/instruções do projeto: DTO explícito, só os campos que a tela realmente usa).
...
- [Source: instruções do projeto — Metodologia XP + TDD, não vender o mesmo assento duas vezes (constraint/lock no banco, não só checagem de aplicação), constraints e FKs coerentes com o domínio, Convenções de nomenclatura]
