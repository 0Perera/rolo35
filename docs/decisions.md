...
## O cold start do plano free é de ~3min, e o timeout do front é que se ajusta a ele

- **Decisão**: a API continua no plano `free` do Render. O `REQUEST_TIMEOUT_MS` do cliente HTTP do front sobe de 90s para 240s, e o README passa a anunciar ~3min de espera na primeira chamada em vez de ~1min.
- **Por quê**: o "~1min" que estava documentado vinha da página do Render, não de medição nossa, e estava errado por um fator de três. Rodando a imagem de produção num container com as restrições exatas do plano — `--cpus=0.1 --memory=512m` —, o Spring Boot levou **179s** pra subir; da linha do tempo do log, 83s até o Flyway terminar e outros 96s no que vem depois. O startup é quase todo CPU-bound e single-thread (carga de classes em modo interpretado, metamodel do Hibernate, springdoc varrendo os controllers), então a 10% de um core cada etapa custa dezenas de vezes o normal. Com o teto de 90s, **o próprio front abortava a primeira chamada depois de cada soneca**: a API subia normalmente e a tela mostrava erro de rede. O número documentado e o número do código estavam ambos abaixo da realidade, e o segundo transformava a espera numa falha.
- **Por que não pagar pra resolver**: o `starter` (0.5 vCPU, US$ 7/mês) elimina o problema — não dorme e sobe em ~36s — e chegou a ser adotado antes de ser revertido. A escolha final foi manter o custo em zero e absorver a espera na documentação: é um projeto de avaliação, não um serviço com usuário real, e três minutos declarados com honestidade custam menos que uma assinatura. Trocar de plataforma resolveria de graça (o Cloud Run dá 1 vCPU no free, dez vezes o Render, com *startup CPU boost*), mas exigiria migrar o deploy inteiro e tirar o Postgres do Render perto do fim do prazo — risco desproporcional ao ganho.
- **O que sobrou pendente**: `spring.main.lazy-initialization=true` e CDS na imagem cortariam parte dos 179s sem custo nenhum (a maior fatia é justamente o que vem depois do Flyway). Não foram feitos porque encurtar a espera não muda a natureza dela — enquanto o serviço dormir, ela existe — e o que a tornava perigosa, o timeout curto, já está resolvido.
