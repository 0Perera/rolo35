...
- **Decisão**: os dois limites da janela em que a portaria pode ativar uma sessão como turno passam a vir de `portaria.turno.janela-antes-minutos` e `portaria.turno.janela-depois-horas`, com default `30` e `2` — exatamente os valores que o CAP-8 decidiu. O `docker-compose.yml` e o `.env.example` sobem o ambiente local com `PORTARIA_JANELA_ANTES_MINUTOS=20160` (14 dias). **Atualização (CAP-6):** com o deploy consolidado no blueprint `render.yaml`, produção passou a fixar o mesmo valor alargado, pelo mesmo motivo do local — o seed também nasce dias no futuro lá. A afirmação original desta entrada, de que produção ficava no default, não é mais verdade; o default `30`/`2` só é exercitado pela suíte de testes.
